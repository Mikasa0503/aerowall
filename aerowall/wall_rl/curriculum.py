...
LAUNCH_PRECONTACT_HANDOFF_CASE_VERSION = 3
...
def handoff_actor_role(case: dict) -> str:
    ...
    if version == HANDOFF_CASE_VERSION:
        ...
        role = "recover"
    elif version == LAUNCH_PRECONTACT_HANDOFF_CASE_VERSION:
        role = "launch"
    else:
    ...
    if version in (RECOVER_HANDOFF_CASE_VERSION, LAUNCH_PRECONTACT_HANDOFF_CASE_VERSION) and declared_role != role:
        raise ValueError(f"{role.title()} handoff state must explicitly identify the {role} actor")
    return role
...
def validate_handoff_case(case: dict) -> None:
    """Validate a state-complete AeroWall Launch, Hit, or Recover handoff."""
    import math
    ...
    if role == "launch":
        if (int(case.get("phase", -1)) != 0 or int(case.get("caps", -1)) != 0
                or int(case.get("walls", -1)) != 0):
            raise ValueError("Launch precontact handoff must begin before any cap or wall event in phase 0")
        lead = float(case.get("source_time_to_contact_s", float("nan")))
        if not math.isfinite(lead) or not 0.38 <= lead <= 0.42:
            raise ValueError("Launch precontact handoff must be captured at 0.40 +/- 0.02 seconds to contact")
        source_action = case.get("source_policy_action")
        if (not isinstance(source_action, (list, tuple)) or len(source_action) != 4
                or not all(math.isfinite(float(item)) for item in source_action)):
            raise ValueError("Launch precontact handoff must retain its source deterministic action")
    elif int(case.get("phase", -1)) != 2 or int(case.get("caps", 0)) < 1 or int(case.get("walls", 0)) < 1:
        raise ValueError(f"{role.title()} handoff must begin after an audited cap and wall event")
    expected_skill_id = {"launch": 0, "hit": 1, "recover": 2}[role]
    ...
    if not isinstance(prefix, list):
        raise ValueError("handoff source_event_prefix must be a list")
    if role == "launch":
        if prefix:
            raise ValueError("Launch precontact handoff must have an empty source event prefix")
        return
    if len(prefix) < 2:
