...
本文是带日期的执行记录。当前贡献归属、C350 最佳方案和拓展工作结论以 [docs/RESEARCH_STATUS.md](docs/RESEARCH_STATUS.md) 与 [docs/RESULTS.md](docs/RESULTS.md) 为准；本文件中的旧路径、旧检查点和旧实验结论仅供历史追溯，不作为当前复现步骤。

# AeroWall 执行状态

## 当前状态：2026-09-25（优先于下方历史记录）

- Part 1–3、11–12 首轮分析与工程落地已完成；Part 4–10、13 仍有能力验证或条件工作未闭环。已完成 Recover Tanh 映射消融（负向）及 phase-0 Launch 横向动量反事实 screen。
- Launch screen 按预注册 12 对开发状态执行：intervention 合法首触 6/12、control 3/12；配对径向误差中位改善 0.0279 m；双方首触 12/12 经 PhysX 核验，reset audit 20 项字段均为零误差。该结果仅为定向开发样本机制信号；intervention 有 2 球越过 world-y ±3 m 边界而 out-of-bounds（control 0；一球终止前已有 4 次 rallies）。
- P2 gate 仍为 false；正式 C350 保持。该诊断不授权训练／晋升，P3–P6 继续 gated。所有协议、哈希与限制见 `docs/part1-14-progress.md` 与 `artifacts/wall-skill-upgrade-v3/launch-diagnostics/analysis.json`。


## 当前状态：2026-09-23（优先于下方历史记录）

- 用户验收通过的最终演示已整理至 `media/AeroWall-wall-rally-demo.mp4`；视频为 1280×720、25 fps、44.28 秒，包含早期 1 轮、中期 2 轮和最终 25 轮三个阶段，无字幕、片头片尾或速度变化。
- 最终回放使用演示种子 5001，完成 25 个有效回合，记录 26 次拍面接触和 26 次墙面接触。接触审计通过；策略第 1840 步球触地后自然结束，没有撞上 4,800 步安全上限。该数字描述指定演示回放，不作为跨种子可靠性结论。
- 最终视频 SHA-256：`d9149f13bebe7858fcf880d20b778c6a042674ae432528faa4b2c7250dcc3ab7`。验收记录见 `docs/demo-acceptance-2026-09-23.md`。
- 发布快照采用稳定验收分支，机器专属路径已替换为仓库相对路径或环境变量；GitHub 版本纳入最终小体积视频，不纳入原始 rollout、训练日志、检查点、仿真缓存或 Isaac Sim 运行时。下方日期更早的条目保留为实验历史。
