...
全阶段 PPO 第 25 次更新固定评测 `distilled-wall-ppo-eval-25-01` 已完成，进程退出、报告及独立事件链审计通过：30/100 场碰墙、0/100 接回、355 次合法拍面接触；11 时限截断、23 球碰机体、33 越界、17 非拍面、13 无人机碰墙、3 触地。相同开发场景下，初始化拟合策略为 48 场碰墙、3 场接回、173 次拍面接触；新增 PPO 后出现退化，不能将训练回合数解释为泛化改善。累计环境步 34062336，包含来源预算。100 次更新训练仍在继续，最新核验到第 40 次；后续 checkpoint 尚无成绩。证据：`distilled-wall-ppo-eval-25-01-audit.json`、`distilled-wall-ppo-comparison.json`。

第 50 次全阶段 PPO 固定评测已完成并通过退出/报告及事件链审计：5/100 场碰墙、0/100 接回、1062 次合法拍面接触。失败/结束原因为球碰机体25、时限20、越界36、触地2、非拍面16、无人机碰墙1。相较第25次30场碰墙进一步退化，接触增加不能称为壁球改善。累计环境步34267136。证据 `distilled-wall-ppo-eval-50-01-audit.json`、`distilled-wall-ppo-comparison-50.json`。
