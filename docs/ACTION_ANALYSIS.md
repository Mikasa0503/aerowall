...
## Exactly matched fast-impact timestep fixtures

The matched-initial-state variant removes the dt-dependent warmup. All recorded drone positions/quaternions/velocities, articulation joint positions/velocities, ball positions/velocities and ball restitution values match exactly across dt=0.02, 0.01, 0.005 and 0.0025 s. In each run, 16 pitches span 20–50 degrees and the ball approaches at 8 m/s along the bat normal. dt=0.02 yields zero legal cap-then-wall cases; the other three yield 16, with 14 occurring before drone-ground termination. All yield zero full rallies. CCD explicitly enabled on scene, ball and bat still yields zero legal deliveries at dt=0.02. Authored CCD flags were read back; this is the result of this scene, not a universal statement about CCD.

Exact initial-state equality is checked by scripts/compare_wall_delivery.py. At the common time 0.1 s, maximum ball-position differences are 0.3189 m (20→10 ms), 0.04370 m (10→5 ms), and 0.05737 m (5→2.5 ms). Thus finer-step contact classification is consistent, but position convergence is not yet established; 5 ms must not be called a validated final physics choice. First-impact velocity samples occur at different timestamps, so their raw vector differences are not instantaneous restitution errors. A 1.25 ms matched run is added for further diagnosis.

The fixed bat joint translation is zero in the pinned asset, so initialization places its root at the drone root without integrating a warmup fall. These fixtures are not closed-loop return policies. Main-policy control frequency must be held separately from physics substeps in future comparisons.

The 1.25 ms run completes with identical recorded initial state, 16 cap-then-wall observations, 14 walls before failure and zero full rallies. Compared with 2.5 ms, the maximum ball-position difference at 0.1 s is 0.009900 m, and the maximum first measured wall-contact-point difference is 0.01451 m across 16 paired cases. For comparison, wall-point differences are 0.11185 m (10→5 ms) and 0.07915 m (5→2.5 ms). This supports 2.5 ms as a candidate for subsequent closed-loop testing, with 1.25 ms retained as the validation reference; it is not a frozen broad accuracy gate across arbitrary speeds, off-center hits or all body contacts. Wall-point statistics include all physical observations, including two after task termination, and cannot be read as task-success statistics.

## Closed-loop substep cycle

The original 32,768,000-frame checkpoint now runs with body collision enabled, aligned physical bat and 2.5 ms physics while policy/PID remain 50 Hz. It produces five legal cap contacts in 96/100 fixed development scenarios and reaches 10 s in 95. The median-duration scenario 48 has 16 legal contacts. Its first contact is at 0.3725 s, contact departure at 0.3775 s, and the next legal contact 0.7075 s after the first (0.7025 s after departure). These are contact-bounded return intervals including flight, not hover settling times. The figure uses 20 ms state samples and physical-substep event times; contact kinematics are captured at the actual physical event. See artifacts/aligned-original-substep8-cycle.png/.json.

The plotting utility now selects departure using physical event timestamps, not policy-step indices: contact and departure can occur within the same policy interval. No fixed contact duration is substituted when an actual LOST event exists. The inspected figure shows the complete first approach/contact/outgoing/return cycle, not just a collision frame. Explicit wall-return and controlled perturbation recovery remain outstanding.
