...
for path in a.evaluation:
    ...
    rows.append({'evaluation':str(path),'report_sha256':hashlib.sha256(path.read_bytes()).hexdigest(),
                 ...
                 'two_rally_streak_scenarios':sum(o['max_streak']>=2 for o in outcomes),
                 'total_rallies':sum(o['rallies'] for o in outcomes),
                 'total_wall_hits':sum(o['wall_hits'] for o in outcomes),
                 'rallies_mean':float(np.mean([o['rallies'] for o in outcomes])),
                 'streak_mean':float(np.mean([o['max_streak'] for o in outcomes])),
                 'streak_median':float(np.median([o['max_streak'] for o in outcomes])),
                 'streak_max':max(o['max_streak'] for o in outcomes),
                 'streak_histogram':{str(k):sum(o['max_streak']==k for o in outcomes) for k in sorted({o['max_streak'] for o in outcomes})},
