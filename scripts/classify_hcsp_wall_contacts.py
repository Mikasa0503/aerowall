...
for env in range(len(r['outcomes'])):
 ...
 for step in np.where(z['contact_entry'][:,env].any(-1)&z['active'][:,env])[0]:
  ...
  wallvalid=bool(kind1) and all(abs((np.array(p['position'])-origins[env])[0]-front)<.06 and abs(p['normal'][0])>=.9 for p in kind1)
...
result={'status':'passed','scope':'Native IrisTest top-face contact candidates; not certified AeroWall bat legality',
 ...
 'criterion':{'normal_abs_z_min':.9,'point_z_min':0,'box_xy_with_motion_tolerance':.255,'wall_point_x':front,'wall_point_tolerance':.06},
