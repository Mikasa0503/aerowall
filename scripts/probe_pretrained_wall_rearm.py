...
def main():
    ...
    try:
        ...
        if args.same_side:
            cfg.task.initial.ball_pos_dist.low[1]=-2.3
            cfg.task.initial.ball_pos_dist.high[1]=-.5
            record(initial_state_distribution='Original Set distribution conditional on ball y in [-2.3,-0.5]; added ball-ground termination')
        ...
        class WallSetReference(parent):
            def _design_scene(self):
                ...
                FixedCuboid('/World/envs/env_0/reference_wall',translation=np.array([2.5,.5,4.]),scale=np.array([8.,.2,8.]),size=1.,physics_material=material)
                return paths
        ...
        if args.skill=='set':
            ...
            names=['checkpoint_secpass.pt','checkpoint_secpass_hover.pt']
        ...
        if args.same_side:
            assert bool((initial_ball[:,1]+base.ball_radius<.4).all())
            assert bool((initial_drone[:,1]+.5<.4).all())
