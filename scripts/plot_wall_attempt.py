...
terminal=outcome.get('physics_steps',outcome['steps'])*r['physics_dt']
keep=active & (t>=max(0,launch['time']-.3)) & (t<terminal-1e-8)
...
markers=[(launch,'launch cap','green'),(wall,'wall','purple')]
if returned is not None and returned['time']<terminal:markers.append((returned,'return cap','darkorange'))
for event,label,color in markers:
...
for axis in [ax[0,1],ax[1,0],ax[1,1]]:
    for event,label,color in markers:axis.axvline(event['time'],color=color,ls='--',label=label)
    axis.axvline(terminal,color='red',ls=':',label='terminal collision')
    axis.set_xlabel('episode time (s)');axis.grid(alpha=.2)
fig.suptitle(f"Scenario {i}: cap -> wall -> {'return cap -> ' if returned is not None else ''}{outcome['reason']}; scored rallies = {outcome['rallies']}")
...
summary={'scenario_id':i,'launch_time':launch['time'],'wall_time':wall['time'],
         ...
         'scope':'Actual first episode; terminal-impact and later samples excluded; absence of next legal cap is not successful recovery; no injected states'}
