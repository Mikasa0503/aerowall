...
def main():
    ...
    try:
        ...
        if a.contacts:
            ...
            for idx in range(a.num_envs):
                view=RigidContactView(f'/World/envs/env_{idx}/ball',
                    [f'/World/envs/env_{idx}/{base.drone.name}_{j}/base_link' for j in range(base.drone.n)],
                ...
                view.initialize();assert view.num_shapes==1 and view.num_filters==base.drone.n
                views.append(view)
        ...
        outcomes=[];trace={'ball':[],'drone':[],'high':[],'active':[],'hits':[],'ball_velocity':[],'executed_high':[]}
        with torch.no_grad():
            td=env.reset()
            for step in range(a.steps):
                ...
                for idx,view in enumerate(views):
                    ...
                    for agent in range(base.drone.n):
