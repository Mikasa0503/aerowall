...
assert (initial_ball[:,1]+.1<.4).all() and (initial_drone[:,1]+.5<.4).all()
...
for outcome in r['outcomes']:
 ...
 for x in s:
  ...
  if kind=='wall':
   ...
   if min(abs(point[1]-.4),abs(point[1]-.6))>.15 or abs(x['normal'][1])<.8:
...
result={'status':'passed','report':str(a.report),'report_sha256':hashlib.sha256(a.report.read_bytes()).hexdigest(),
