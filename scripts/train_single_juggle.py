"""Development SingleJuggle training with upstream PPO/CTBR and durable evidence.
...
def main():
    ...
    report = {'status': 'initializing', 'pid': os.getpid(), 'phase': 'development_singlejuggle',
              ...
              'source_hashes': {str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest()
                                for p in [Path(__file__), ROOT / 'scripts/runtime_adapters.py', ROOT / 'scripts/python.sh']}}
    def record(**values):
    ...
    try:
        ...
        for index, data in enumerate(collector):
            ...
            row = {'update_this_run': completed, 'total_updates': policy.n_updates,
                   ...
                   'upstream_episode_stat_means': stats, 'metrics': info,
                   'elapsed_seconds': time.monotonic() - started, 'rollout_fps': collector._fps}
